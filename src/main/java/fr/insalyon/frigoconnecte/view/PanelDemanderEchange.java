...
public class PanelDemanderEchange extends StandardPanel {
     ...
     */
    public PanelDemanderEchange(Events eventsHandler, int idFrigo) {
        super();
        this.setLayout(null);
        ...
        try {
            drawExchangeInventaryFromResultSet(Main.maBD.getExchangeProductsFor(idFrigo));
    ...
    public void drawExchangeInventaryFromResultSet(ResultSet rs) throws SQLException {
        ...
        for (i = 0; i < produits.size(); i++) {
            MiniPanelProduit mpp = new MiniPanelProduit(produits.get(i), true, this.idFrigo);
